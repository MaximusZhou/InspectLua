...
template < class T > class Luna {
	public:
		...
		static T* check(lua_State * L, int narg)
		{
			...
			return *obj;        // return pointer to T object
		}
		...
		*/
		...
		static void Register(lua_State * L, const char *namespac = NULL ) {
			...
			// To be able to compare two Luna objects
			...
			for (int i = 0; T::properties[i].name; i++) { 				// Register some properties in it
				lua_pushstring(L, T::properties[i].name);               //it Having some string associated with them
				lua_pushnumber(L, i); 							    // And a number indexing which property it is
				lua_settable(L, metatable);                         //metatable[i] = propertyfunc
			}
			...
			for (int i = 0; T::methods[i].name; i++) {
				...
				lua_settable(L, metatable);                           //metatable[i] = method
			}
		}
		...
		static int constructor(lua_State * L)
		{
			...
			*a = ap;
			...
			return 1;  //return a userdata type to lua
		}
